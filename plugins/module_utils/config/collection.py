...
class CollectionConfigBase(ConfigBase):
    ...
    records (syslog servers, SNMP alert managers, ...) that support full
    add/update/remove semantics via POST/PUT/DELETE, unlike simpler
    append-only collections (e.g. authorized keys).
